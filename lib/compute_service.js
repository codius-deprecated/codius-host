...
// TODO: Emit 'running' from codius-engine
    ...
    runner.on('exit', function (code, signal) {
      winston.debug(contractIdent, chalk.dim('+++'), 'Instance stopped');
      this_._instances[contractToken].state = 'stopped';
      events.emit('contract:stopped', token);

      // Delete the contract from the filesystem
// TODO: Update codius-engine to handle contract deletion
      // engine.engine.deleteContract(contractHash);
      function deleteFileByHash(hash) {
        ...
        fs.unlink(filePath, function(err) {
          ...
          fs.readdir(secondDir, function(err, files) {
            if (err) throw err;
            if (files.length===0) {
              fs.rmdir(secondDir, function(err){
                if (err) throw err;
                fs.readdir(firstDir, function(err, files) {
                  if (err) throw err;
                  if (files.length===0) {
                    fs.rmdir(firstDir, function(err){
                      if (err) throw err;
                    });
                  }
                });
      ...
      function deleteFile(file) {
        // Files in subdirectories are stored in the manifest as:
        // {subdir_name: {file_name: HASH}}
        if (typeof file==='object') {
          _.each(file, function(subfile) {
            deleteFile(subfile);
          })
        } else {
          deleteFileByHash(file)
        }
      }
      ...
      deleteFile(contractHash);

      winston.debug(contractIdent, chalk.dim('+++'), 'Instance terminated');
      this_._instances[contractToken].state = 'terminated';
    });
